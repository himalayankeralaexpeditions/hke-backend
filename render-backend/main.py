...
GOOGLE_ENQUIRY_SHEET_ID = os.getenv("GOOGLE_ENQUIRY_SHEET_ID", GOOGLE_SHEET_ID).strip()
GOOGLE_BOOKING_SHEET_ID = os.getenv("GOOGLE_BOOKING_SHEET_ID", GOOGLE_SHEET_ID).strip()
GOOGLE_CONTACT_SHEET_ID = os.getenv(
    "GOOGLE_CONTACT_SHEET_ID",
    GOOGLE_ENQUIRY_SHEET_ID or GOOGLE_SHEET_ID
).strip()
...
def get_google_enquiry_sheet_id() -> str:
    return safe_str(GOOGLE_ENQUIRY_SHEET_ID or GOOGLE_SHEET_ID)


def get_google_enquiry_tab() -> str:
    return safe_str(GOOGLE_ENQUIRY_TAB or GOOGLE_SHEET_TAB or "Sheet1", "Sheet1")


def get_google_contact_sheet_id() -> str:
    return safe_str(GOOGLE_CONTACT_SHEET_ID or get_google_enquiry_sheet_id())


def get_google_contact_tab() -> str:
    return safe_str(GOOGLE_CONTACT_TAB or get_google_enquiry_tab(), "Sheet1")


def get_google_booking_sheet_id() -> str:
    return safe_str(GOOGLE_BOOKING_SHEET_ID or GOOGLE_SHEET_ID)


def get_google_booking_tab() -> str:
    return safe_str(GOOGLE_BOOKING_TAB or GOOGLE_SHEET_TAB or "Sheet1", "Sheet1")


def _build_google_sheet_result(
    ok: bool,
    message: str,
    *,
    sheet_id: str = "",
    ...
    if service_account_email:
        result["serviceAccountEmail"] = service_account_email
    if row_count_before is not None:
        result["rowCountBefore"] = row_count_before
    if row_count_after is not None:
        result["rowCountAfter"] = row_count_after
    return result
...
def get_google_sheets_client():
    ...
    if not gspread or not GoogleCredentials:
        logger.error("Google Sheets is not available because gspread/google-auth could not be imported")
        return None
    ...
    try:
        if GOOGLE_SERVICE_ACCOUNT_JSON:
            ...
            if not os.path.exists(GOOGLE_APPLICATION_CREDENTIALS):
                logger.error(
                    "Google Sheets is not configured correctly: GOOGLE_APPLICATION_CREDENTIALS file not found at %s",
            ...
            logger.error(
                "Google Sheets is not configured: set GOOGLE_SERVICE_ACCOUNT_JSON or GOOGLE_APPLICATION_CREDENTIALS"
            )
            return None
    except json.JSONDecodeError:
        logger.exception("Google Sheets configuration is invalid: GOOGLE_SERVICE_ACCOUNT_JSON is not valid JSON")
        return None
    except Exception:
        logger.exception("Google Sheets credentials initialization failed")
        return None
    ...
    try:
        ...
        logger.exception("Google Sheets authorization failed")
        return None
...
def get_google_service_account_email() -> str:
    if GOOGLE_SERVICE_ACCOUNT_JSON:
        try:
            ...
            return safe_str(info.get("client_email"))
        except Exception:
            return ""
    if GOOGLE_APPLICATION_CREDENTIALS and os.path.exists(GOOGLE_APPLICATION_CREDENTIALS):
        try:
            with open(GOOGLE_APPLICATION_CREDENTIALS, "r", encoding="utf-8") as handle:
...
def count_google_sheet_rows(worksheet) -> Optional[int]:
    try:
        values = worksheet.get_all_values()
        return len(values)
    except Exception:
        logger.exception("Google Sheets row count fetch failed")
        return None
...
def get_google_sheet_worksheet(sheet_id: str, tab_name: str):
    ...
    try:
        worksheet = client_obj.open_by_key(sheet_id).worksheet(tab_name)
        GOOGLE_WORKSHEET_CACHE[cache_key] = worksheet
        return worksheet
    except Exception:
        logger.exception(
            "Google Sheets worksheet lookup failed for sheet_id=%s tab=%s",
            sheet_id,
            tab_name
        )
        return None
...
def append_row_to_google_sheet(
    ...
    *,
    ...
    if not sheet_id:
        logger.error("Google Sheets skipped for %s: missing sheet id", context)
        return _build_google_sheet_result(
            False,
            "Google Sheet destination is not configured on server. Please contact support.",
            sheet_id=sheet_id,
            tab_name=tab_name,
            context=context,
    ...
    if not tab_name:
        logger.error("Google Sheets skipped for %s: missing tab name", context)
        return _build_google_sheet_result(
            False,
            "Google Sheet tab name is not configured on server. Please contact support.",
            sheet_id=sheet_id,
            tab_name=tab_name,
            context=context,
            error="missing_tab_name",
            service_account_email=service_account_email
        )
    ...
    if not worksheet:
        logger.error(
            "Google Sheets skipped for %s: worksheet unavailable for sheet_id=%s tab=%s",
            context,
            sheet_id,
            tab_name
        )
        return _build_google_sheet_result(
            False,
            "Unable to connect to the Google Sheet right now. Please try again or contact support.",
            sheet_id=sheet_id,
            tab_name=tab_name,
            context=context,
    ...
    try:
        row_count_before = count_google_sheet_rows(worksheet)
        worksheet.append_row(
            [safe_str(value) for value in row_values],
            value_input_option="USER_ENTERED"
        )
        row_count_after = count_google_sheet_rows(worksheet)
        logger.info(
            "Google Sheets append successful for context=%s sheet_id=%s tab=%s",
            context,
            sheet_id,
            tab_name
        )
        ...
        if "permission" in lower_error or "403" in lower_error or "not have permission" in lower_error:
            error_code = "permission_denied"
            logger.error(
                "Google Sheets permission denied for context=%s sheet_id=%s tab=%s service_account=%s error=%s",
                context,
                sheet_id,
                tab_name,
        ...
        elif "worksheet" in lower_error and "not found" in lower_error:
            error_code = "worksheet_not_found"
            logger.error(
                "Google Sheets worksheet not found for context=%s sheet_id=%s tab=%s",
                context,
                sheet_id,
                tab_name
            )
            ...
            logger.error(
                "Google Sheets append_row failure for context=%s sheet_id=%s tab=%s error=%s",
                context,
                sheet_id,
                tab_name,
                error_text
        ...
        logger.exception(
            "Google Sheets append failed for context=%s sheet_id=%s tab=%s",
            context,
            sheet_id,
            tab_name
        )
        return _build_google_sheet_result(
            False,
            "We received your request, but saving it to Google Sheets failed. Please contact support and check backend logs.",
            sheet_id=sheet_id,
            tab_name=tab_name,
            context=context,
            error=error_code,
...
def append_ai_enquiry_to_google_sheet(data: Dict[str, Any], source: str) -> Dict[str, Any]:
    row = [
        safe_str(data.get("name")),
        clean_phone(safe_str(data.get("phone"))),
        safe_str(data.get("email")),
        safe_str(data.get("fromLocation")),
        safe_str(data.get("destination")),
        safe_str(data.get("startDate")),
        safe_str(data.get("endDate")),
        safe_int(data.get("days")),
        safe_int(data.get("travellers")),
        safe_int(data.get("rooms")),
        safe_str(data.get("budget")),
        safe_str(data.get("hotelClass")),
        safe_str(data.get("travelType")),
        safe_str(data.get("vehicle")),
        safe_str(data.get("guide")),
        ", ".join([safe_str(place) for place in data.get("places") or [] if safe_str(place)]),
        safe_str(data.get("notes")),
        utc_now_iso(),
    ]
    return append_row_to_google_sheet(
        get_google_enquiry_sheet_id(),
        get_google_enquiry_tab(),
        row,
        context="ai_enquiry"
    )


def append_contact_to_google_sheet(data: Dict[str, Any]) -> Dict[str, Any]:
    row = [
        utc_now_iso(),
        safe_str(data.get("name")),
        clean_phone(safe_str(data.get("phone"))),
        safe_str(data.get("email")),
        safe_str(data.get("message")),
        safe_str(data.get("page")),
        safe_str(data.get("source", "website_contact")),
    ]
    return append_row_to_google_sheet(
        get_google_contact_sheet_id(),
        get_google_contact_tab(),
        row,
        context="contact_enquiry"
    )


def append_booking_to_google_sheet(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    row = [
        ...
        safe_str(customer.get("packageName") or data.get("packageName")),
        ...
        safe_float(payment.get("paidAmount")),
        safe_float(pricing.get("finalFare")),
        safe_float(payment.get("remainingAmount")),
        safe_str(payment.get("paymentStatus")),
        safe_str(customer.get("notes")),
    ]
    return append_row_to_google_sheet(
        get_google_booking_sheet_id(),
...
def get_google_sheet_diagnostic() -> Dict[str, Any]:
    service_account_email = get_google_service_account_email()
    sheet_id = get_google_enquiry_sheet_id()
    tab_name = get_google_enquiry_tab()

    result = {
        "hasGoogleServiceAccountJson": bool(GOOGLE_SERVICE_ACCOUNT_JSON),
        "hasGoogleApplicationCredentials": bool(GOOGLE_APPLICATION_CREDENTIALS),
        "googleApplicationCredentialsPathExists": bool(
            GOOGLE_APPLICATION_CREDENTIALS and os.path.exists(GOOGLE_APPLICATION_CREDENTIALS)
        ),
        "sheetIdUsed": sheet_id,
        "tabUsed": tab_name,
        "serviceAccountEmailDetected": bool(service_account_email),
        "serviceAccountEmail": service_account_email,
        "spreadsheetOpenOk": False,
        "worksheetOpenOk": False,
        "testAppendSucceeded": False,
        "error": ""
    }

    client_obj = get_google_sheets_client()
    if not client_obj:
        result["error"] = "google_client_initialization_failed"
        return {"ok": True, "googleSheet": result}
    ...
    try:
        ...
        error_text = safe_str(exc)
        logger.exception("Google Sheets diagnostic failed to open spreadsheet sheet_id=%s", sheet_id)
        result["error"] = error_text or "spreadsheet_open_failed"
        return {"ok": True, "googleSheet": result}

    try:
        worksheet = spreadsheet.worksheet(tab_name)
        result["worksheetOpenOk"] = True
    except Exception as exc:
        error_text = safe_str(exc)
        logger.exception(
            "Google Sheets diagnostic failed to open worksheet sheet_id=%s tab=%s",
            sheet_id,
            tab_name
        )
        result["error"] = error_text or "worksheet_open_failed"
        return {"ok": True, "googleSheet": result}
    ...
    try:
        ...
        worksheet.append_row(
            [
                ...
                "Diagnostic",
                "Diagnostic Place",
                "Diagnostic append from /api/debug/google-sheet",
                utc_now_iso(),
            ],
            value_input_option="USER_ENTERED"
        )
        row_count_after = count_google_sheet_rows(worksheet)
        result["testAppendSucceeded"] = True
        result["rowCountBefore"] = row_count_before
        result["rowCountAfter"] = row_count_after
        return {"ok": True, "googleSheet": result}
    except Exception as exc:
        error_text = safe_str(exc)
        logger.exception(
            "Google Sheets diagnostic test append failed sheet_id=%s tab=%s",
            sheet_id,
            tab_name
        )
        result["error"] = error_text or "test_append_failed"
        return {"ok": True, "googleSheet": result}
...
class ContactLeadRequest(BaseModel):
    ...
    message: str
    page: Optional[str] = "contact.html"
...
class GoogleSheetTestAppendRequest(BaseModel):
    name: str
    phone: str
    email: Optional[EmailStr] = None
    message: str
...
def generate_itinerary(payload: PlannerRequest):
    ...
    google_sheet = append_ai_enquiry_to_google_sheet(data, source)
    ...
    return {
        ...
        "googleSheet": {
            **google_sheet,
            "error": safe_str(google_sheet.get("error")),
        }
    }
...
def submit_contact_or_lead(payload: ContactLeadRequest):
    ...
    google_sheet = append_contact_to_google_sheet(data)
    if not google_sheet.get("ok"):
        raise HTTPException(status_code=502, detail=google_sheet.get("message"))
...
@app.get("/api/debug/google-sheet")
def debug_google_sheet():
    return get_google_sheet_diagnostic()


@app.post("/api/debug/google-sheet/test-append")
def debug_google_sheet_test_append(payload: GoogleSheetTestAppendRequest):
    data = payload.model_dump()
    row = [
        safe_str(data.get("name")),
        clean_phone(safe_str(data.get("phone"))),
    ...
    google_sheet = append_row_to_google_sheet(
        get_google_enquiry_sheet_id(),
        get_google_enquiry_tab(),
        row,
        context="debug_test_append"
    )
    return {
        "ok": bool(google_sheet.get("ok")),
        "googleSheet": google_sheet
    }
...
def save_payment_confirmation(payload: SavePaymentRequest):
    ...
    google_sheet = append_booking_to_google_sheet({
    ...
    return {
        "ok": True,
        "message": "Payment confirmation saved successfully",
        "bookingRef": booking_ref,
        "googleSheet": google_sheet
    }
